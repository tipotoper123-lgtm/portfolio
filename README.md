...
Единственное место настройки — `config.js`, поле `telegramUsername`. Установлен подтверждённый рабочий контакт `nikita_automates`: https://t.me/nikita_automates . Из этой настройки формируются кнопки первого экрана и контактов, а также ссылка `@nikita_automates`.

При смене аккаунта укажите подтверждённый рабочий username без `@`. Пустое или некорректное значение скрывает ссылки. Не помещайте сюда токены бота, API-ключи или другие секреты. Проверка формата не подтверждает принадлежность аккаунта.
